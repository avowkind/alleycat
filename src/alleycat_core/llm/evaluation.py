...
class LLMTestCase(BaseModel):
    ...
    expected_patterns: List[str] = Field(
        ...
        description="List of regex patterns or exact matches to look for"
    )
    required_elements: List[str] = Field(
        default_factory=list,
        description="List of elements that must be present in the response"
    )
    forbidden_elements: List[str] = Field(
        default_factory=list,
        description="List of elements that must not be present in the response"
    )
    min_score: float = Field(
        default=0.8,
        ge=0.0,
        le=1.0,
        description="Minimum score required for the test to pass"
    )
    tools: Optional[List[str]] = Field(
        default=None,
        description="List of tools that should be available"
    )
    ...
    settings: dict[str, Any] = Field(
        ...
        description="Additional settings for the LLM (temperature, etc.)"
    )
...
class ResponseEvaluator:
    ...
    def evaluate(self, response_text: str, test_case: LLMTestCase) -> ResponseEvaluation:
        ...
        final_score = (
            (pattern_score * self.exact_match_weight + 
             required_score * self.semantic_match_weight) *

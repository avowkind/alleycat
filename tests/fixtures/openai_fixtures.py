...
def mock_openai_response() -> OpenAIResponse:
    ...
    """
    message_content = ResponseOutputText(type="output_text", text="42", annotations=[])
...
async def mock_openai_client(
    mock_openai_response: OpenAIResponse, monkeypatch: pytest.MonkeyPatch
    ...
    """
    config = OpenAIConfig(api_key="sk-test-key", model="gpt-4o-mini", temperature=0.7)
    ...
    async def mock_create(**kwargs: Any) -> OpenAIResponse:
        ...
        ), "Temperature should match config"

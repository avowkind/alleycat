...
from tests.alleycat_core.llm.test_utils import create_test_case, load_test_cases, load_test_suite


@pytest.fixture
def test_yaml_file() -> Generator[str, None, None]:
    """Create a temporary YAML file with test cases."""
    test_data = {
        "basic_tests": [
            {"name": "test1", "prompt": "Return 42", "expected_patterns": ["42"]},
            {
                "name": "test2",
                "prompt": "Return JSON",
...
def test_load_test_cases(test_yaml_file: str) -> None:
...
def test_load_test_suite(test_yaml_file: str) -> None:
...
def test_create_test_case_defaults() -> None:
    """Test test case creation with default values."""
    test_case = create_test_case(name="minimal_test", prompt="Return 42", expected_patterns=["42"])
